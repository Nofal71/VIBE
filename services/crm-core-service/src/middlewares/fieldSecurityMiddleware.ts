...
export const applyFieldSecurity = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const roleId = (req as any).user?.roleId || 'default-role-id';
    ...
    try {
        ...
        const restrictedKeys = locks
            .filter((lock: any) => lock.can_read === false && lock.table_name === tableName)
            .map((lock: any) => lock.column_name);
        ...
        res.json = function (body: any): Response {
            ...
            if (restrictedKeys.length > 0 && body) {
                ...
                const scrubObject = (obj: any) => {
                    restrictedKeys.forEach((key: string) => {
                ...
                if (Array.isArray(body.records)) {
                    body.records = body.records.map((item: any) => scrubObject({ ...item }));
                } else if (Array.isArray(body)) {
                    body = body.map((item: any) => scrubObject({ ...item }));
