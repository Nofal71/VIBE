...
exports.ImportExportController = exports.csvUpload = void 0;
...
exports.csvUpload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage() });
class ImportExportController {
    ...
    static async exportLeads(req, res) {
        try {
            ...
            const leads = await sequelize.query('SELECT * FROM `leads`', { type: sequelize_1.QueryTypes.SELECT });
            if (!leads.length) {
            ...
            const rows = leads.map((row) => Object.values(row)
                .map((v) => `"${String(v ?? '').replace(/"/g, '""')}"`)
                .join(','));
            const csv = [headers, ...rows].join('\r\n');
            res.setHeader('Content-Type', 'text/csv');
            res.setHeader('Content-Disposition', `attachment; filename="leads_${tenantId}_${Date.now()}.csv"`);
    ...
    static async importLeads(req, res) {
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const id = rowData['id'] || (0, uuid_1.v4)();
                const insertData = { id, ...rowData };
                const cols = Object.keys(insertData).map((k) => `\`${k}\``).join(', ');
                const placeholders = Object.keys(insertData).map(() => '?').join(', ');
                const vals = Object.values(insertData);
                await sequelize.query(`INSERT IGNORE INTO \`leads\` (${cols}) VALUES (${placeholders})`, { replacements: vals, type: sequelize_1.QueryTypes.INSERT });
                imported++;
            }
